...
namespace Visitor.Persistence.Repositories
{
    public class ManageReportRepository : GenericRepository, IManageReportRepository
    {
        ...
        public async Task<IEnumerable<SafetyReport_Response>> GetSafetyReport(SafetyReport_Search parameters)
        {
            DynamicParameters queryParameters = new DynamicParameters();
            queryParameters.Add("@FromDate", parameters.FromDate);
            queryParameters.Add("@ToDate", parameters.ToDate);
            ...
            var result = await ListByStoredProcedure<SafetyReport_Response>("GetSafetyReport", queryParameters);
            parameters.Total = queryParameters.Get<int>("Total");
