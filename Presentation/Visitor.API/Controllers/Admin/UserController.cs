...
namespace Visitor.API.Controllers.Admin
{
    ...
    public class UserController : CustomBaseController
    {
        ...
        public async Task<ResponseModel> GetUserList(User_Search parameters)
        {
            ...
            foreach(var items in lstUsers)
            {
                var gateNolistObj = await _assignGateNoRepository.GetAssignGateNoById(items.Id, items.UserTypeId == 1 ? "Employee" : "Security", 0);
        ...
        public async Task<ResponseModel> ImportUser([FromQuery] ImportRequest request)
        {
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                if (!string.Equals(workSheet.Cells[1, 1].Value.ToString(), "UserCode", StringComparison.OrdinalIgnoreCase) ||
                    ...
                    !string.Equals(workSheet.Cells[1, 11].Value.ToString(), "Branch", StringComparison.OrdinalIgnoreCase) ||
                ...
                for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                {
                    if (!string.IsNullOrWhiteSpace(workSheet.Cells[rowIterator, 2].Value?.ToString()) && !string.IsNullOrWhiteSpace(workSheet.Cells[rowIterator, 3].Value?.ToString()))
                    {
                        lstUser_ImportData.Add(new User_ImportData()
                        {
                            ...
                            Branch = workSheet.Cells[rowIterator, 11].Value?.ToString(),
        ...
        private byte[] GenerateInvalidImportDataFile(IEnumerable<User_ImportDataValidation> lstUser_ImportDataValidation)
        {
            ...
            using (MemoryStream msInvalidDataFile = new MemoryStream())
            {
                using (ExcelPackage excelInvalidData = new ExcelPackage())
                {
                    ...
                    WorkSheet1.Cells[1, 6].Value = "GateNumber";

                    WorkSheet1.Cells[1, 7].Value = "Role";
                    WorkSheet1.Cells[1, 8].Value = "ReportingTo";
                    WorkSheet1.Cells[1, 9].Value = "Department";
                    WorkSheet1.Cells[1, 10].Value = "Company";
                    WorkSheet1.Cells[1, 11].Value = "Branch";
                    ...
                    foreach (User_ImportDataValidation record in lstUser_ImportDataValidation)
                    {
                        ...
                        WorkSheet1.Cells[recordIndex, 6].Value = record.GateNumber;
                        ...
                        WorkSheet1.Cells[recordIndex, 11].Value = record.Branch;
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> EmployeeApproveNReject(Employee_ApproveNReject parameters)
        {
            if (parameters.Id <= 0)
            {
                _response.Message = "Id is required";
            ...
            {
                int resultExpenseDetails = await _userRepository.EmployeeApproveNReject(parameters);
                ...
                else
                {
                    ...
                    {
                        _response.Message = "Employee Approved successfully.";
                    }
                    else if (parameters.StatusId == 3)
                    {
                        _response.Message = "Employee Rejected successfully.";
                    }
                }
            }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> ExportEmployeeLiveReportData(User_Search parameters)
        {
            _response.IsSuccess = false;
            byte[] result;
            int recordIndex;
            ...
            using (MemoryStream msExportDataFile = new MemoryStream())
            {
                using (ExcelPackage excelExportData = new ExcelPackage())
                {
                    WorkSheet1 = excelExportData.Workbook.Worksheets.Add("EmployeeLiveReport");
                    WorkSheet1.TabColor = System.Drawing.Color.Black;
                    WorkSheet1.DefaultRowHeight = 12;
                    ...
                    foreach (var items in lstSizeObj)
                    {
                        ...
                        if (parameters.IsLiveReport == true)
                        {
                            vCheckedInOutLogHistory_Search.IsLiveReport = parameters.IsLiveReport;
                            vCheckedInOutLogHistory_Search.FromDate = parameters.FromDate;
                            vCheckedInOutLogHistory_Search.GateDetailsId = parameters.GateDetailsId;
                        }
                        ...
                        if (lstMUserObj.ToList().Count > 0)
                        {
                            foreach (var mitems in lstMUserObj.ToList().Take(1))
                            {
                                if (j == 0)
                                {
                                    WorkSheet1.Cells[recordIndex, 1].Value = i.ToString();
