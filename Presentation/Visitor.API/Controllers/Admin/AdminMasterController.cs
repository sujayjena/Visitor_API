...
namespace Visitor.API.Controllers.Admin
{
    ...
    public class AdminMasterController : CustomBaseController
    {
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> ExportFoodItemData(FoodItem_Search_Request parameters)
        {
            ...
            IEnumerable<FoodItem_Response> lstObj = await _adminMasterRepository.GetFoodItemList(parameters);

            using (MemoryStream msExportDataFile = new MemoryStream())
            {
                using (ExcelPackage excelExportData = new ExcelPackage())
                {
                    WorkSheet1 = excelExportData.Workbook.Worksheets.Add("FoodItem");
                    ...
                    WorkSheet1.Cells[1, 1].Value = "Days";
                    WorkSheet1.Cells[1, 2].Value = "Canteen Name";
                    WorkSheet1.Cells[1, 3].Value = "Food Item";
                    WorkSheet1.Cells[1, 4].Value = "Is Veg";
                    WorkSheet1.Cells[1, 5].Value = "Is Subsidised";
                    ...
                    {
                        var vSearchRequest = new FoodItemDays_Search_Request();
                        vSearchRequest.FoodItemId = items.Id;

                        var vDayList = await _adminMasterRepository.GetFoodItemDaysList(vSearchRequest);

                        string strDaysList = string.Empty;
                        if (vDayList.ToList().Count > 0)
                        {
                            strDaysList = string.Join(",", vDayList.ToList().Select(x => x.DaysName));
                        }

                        WorkSheet1.Cells[recordIndex, 1].Value = strDaysList;
                        WorkSheet1.Cells[recordIndex, 2].Value = items.CanteenName;
                        WorkSheet1.Cells[recordIndex, 3].Value = items.MenuItemName;
                        WorkSheet1.Cells[recordIndex, 4].Value = items.IsVeg == true ? "Yes" : "No";
                        WorkSheet1.Cells[recordIndex, 5].Value = items.IsSubsidized == true ? "Yes" : "No";
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> SaveGrocery(Grocery_Request parameters)
        {
            int result = await _adminMasterRepository.SaveGrocery(parameters);
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetGroceryList(Grocery_Search parameters)
        {
            IEnumerable<Grocery_Response> lstRoles = await _adminMasterRepository.GetGroceryList(parameters);
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetGroceryById(int Id)
        {
            if (Id <= 0)
            {
                _response.Message = "Id is required";
            ...
            {
                var vResultObj = await _adminMasterRepository.GetGroceryById(Id);
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> DownloadGroceryTemplate()
        {
            byte[]? formatFile = await Task.Run(() => _fileManager.GetFormatFileFromPath("Template_Grocery.xlsx"));
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> ImportGrocery([FromQuery] ImportRequest request)
        {
            _response.IsSuccess = false;
            List<string[]> data = new List<string[]>();
            ExcelWorksheets currentSheet;
            ExcelWorksheet workSheet;
            List<Grocery_ImportData> lstImportedGrocery = new List<Grocery_ImportData>();
            IEnumerable<Grocery_ImportDataValidation> lstGroceryFailedToImport;
            ExcelPackage.LicenseContext = OfficeOpenXml.LicenseContext.NonCommercial;
            int noOfCol, noOfRow;

            if (request.FileUpload == null || request.FileUpload.Length == 0)
            {
                _response.Message = "Please upload an excel file to import Grocery data";
                return _response;
            }
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                if (!string.Equals(workSheet.Cells[1, 1].Value.ToString(), "GroceryName", StringComparison.OrdinalIgnoreCase) ||
                    !string.Equals(workSheet.Cells[1, 2].Value.ToString(), "GroceryDesc", StringComparison.OrdinalIgnoreCase) ||
                    ...
                    !string.Equals(workSheet.Cells[1, 5].Value.ToString(), "AvailableQty", StringComparison.OrdinalIgnoreCase) ||
                ...
                {
                    lstImportedGrocery.Add(new Grocery_ImportData()
                    {
                        GroceryName = workSheet.Cells[rowIterator, 1].Value?.ToString(),
                        GroceryDesc = workSheet.Cells[rowIterator, 2].Value?.ToString(),
                        UOM = workSheet.Cells[rowIterator, 3].Value?.ToString(),
                        MinQty = workSheet.Cells[rowIterator, 4].Value?.ToString(),
                        AvailableQty = workSheet.Cells[rowIterator, 5].Value?.ToString(),
                        IsActive = workSheet.Cells[rowIterator, 6].Value?.ToString()
                    });
                }
            }

            if (lstImportedGrocery.Count == 0)
            {
                _response.Message = "File does not contains any record(s)";
                return _response;
            }

            lstGroceryFailedToImport = await _adminMasterRepository.ImportGrocery(lstImportedGrocery);

            if (lstGroceryFailedToImport.ToList().Count > 0)
            {
                _response.IsSuccess = false;
                _response.Message = "Uploaded file contains invalid records, please check downloaded file for more details";
                _response.Data = GenerateInvalidGroceryDataFile(lstGroceryFailedToImport);
            }
            else
            {
                _response.IsSuccess = true;
                _response.Message = "Grocery list imported successfully";
            }
            
            return _response;
        }

        private byte[] GenerateInvalidGroceryDataFile(IEnumerable<Grocery_ImportDataValidation> lstGroceryFailedToImport)
        {
            byte[] result;
            int recordIndex;
            ExcelWorksheet WorkSheet1;
            ...
            using (MemoryStream msInvalidDataFile = new MemoryStream())
            {
                ...
                {
                    WorkSheet1 = excelInvalidData.Workbook.Worksheets.Add("Invalid_Grocery_Records");
                    WorkSheet1.TabColor = System.Drawing.Color.Black;
                    WorkSheet1.DefaultRowHeight = 12;
                    ...
                    WorkSheet1.Cells[1, 2].Value = "GroceryDesc";
                    WorkSheet1.Cells[1, 3].Value = "UOM";
                    WorkSheet1.Cells[1, 4].Value = "MinQty";
                    WorkSheet1.Cells[1, 5].Value = "AvailableQty";
                    WorkSheet1.Cells[1, 6].Value = "IsActive";
                    WorkSheet1.Cells[1, 7].Value = "ValidationMessage";

                    recordIndex = 2;

                    foreach (Grocery_ImportDataValidation record in lstGroceryFailedToImport)
                    {
                        WorkSheet1.Cells[recordIndex, 1].Value = record.GroceryName;
                        WorkSheet1.Cells[recordIndex, 2].Value = record.GroceryDesc;
                        WorkSheet1.Cells[recordIndex, 3].Value = record.UOM;
                        WorkSheet1.Cells[recordIndex, 4].Value = record.MinQty;
                        WorkSheet1.Cells[recordIndex, 5].Value = record.AvailableQty;
        ...
        public async Task<ResponseModel> ExportGroceryData(Grocery_Search request)
        {
            ...
            using (MemoryStream msExportDataFile = new MemoryStream())
            {
                using (ExcelPackage excelExportData = new ExcelPackage())
                {
                    WorkSheet1 = excelExportData.Workbook.Worksheets.Add("Grocery");
                    WorkSheet1.TabColor = System.Drawing.Color.Black;
                    WorkSheet1.DefaultRowHeight = 12;
                    ...
                    WorkSheet1.Cells[1, 1].Value = "GroceryName";
                    WorkSheet1.Cells[1, 2].Value = "GroceryDesc";
                    WorkSheet1.Cells[1, 3].Value = "UOMName";
                    WorkSheet1.Cells[1, 4].Value = "MinQty";
                    WorkSheet1.Cells[1, 5].Value = "AvailableQty";
                    ...
                    foreach (var items in lstGroceryObj)
                    {
                        ...
                        WorkSheet1.Cells[recordIndex, 2].Value = items.GroceryDesc;
            ...
            if (result != null)
            {
                _response.Data = result;
                _response.IsSuccess = true;
                _response.Message = "Grocery list Exported successfully";
            }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> SaveVessel(Vessel_Request parameters)
        {
            int result = await _adminMasterRepository.SaveVessel(parameters);
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetVesselList(BaseSearchEntity parameters)
        {
            IEnumerable<Vessel_Response> lstRoles = await _adminMasterRepository.GetVesselList(parameters);
            _response.Data = lstRoles.ToList();
            _response.Total = parameters.Total;
            return _response;
        }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetVesselById(int Id)
        {
            if (Id <= 0)
            {
                _response.Message = "Id is required";
            ...
            {
                var vResultObj = await _adminMasterRepository.GetVesselById(Id);
                _response.Data = vResultObj;
            }
            return _response;
        }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> SaveCanteenMachine(CanteenMachine_Request parameters)
        {
            int result = await _adminMasterRepository.SaveCanteenMachine(parameters);
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetCanteenMachineList(CanteenMachine_Search parameters)
        {
            IEnumerable<CanteenMachine_Response> lstRoles = await _adminMasterRepository.GetCanteenMachineList(parameters);
            _response.Data = lstRoles.ToList();
            _response.Total = parameters.Total;
            return _response;
        }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetCanteenMachineById(int Id)
        {
            if (Id <= 0)
            {
                _response.Message = "Id is required";
            ...
            {
                var vResultObj = await _adminMasterRepository.GetCanteenMachineById(Id);
                _response.Data = vResultObj;
            }
            return _response;
        }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> UpdateCanteenMachineUsedStatus(UpdateCanteenMachineUsedStatus_Request parameters)
        {
            int result = await _adminMasterRepository.UpdateCanteenMachineUsedStatus(parameters);
