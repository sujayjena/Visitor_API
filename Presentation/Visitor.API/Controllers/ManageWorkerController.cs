...
namespace Visitor.API.Controllers
{
    ...
    public class ManageWorkerController : CustomBaseController
    {
        ...
        public async Task<ResponseModel> ExportWorkerAttendanceData(WorkerSearch_Request parameters)
        {
            ...
            using (MemoryStream msExportDataFile = new MemoryStream())
            {
                using (ExcelPackage excelExportData = new ExcelPackage())
                {
                    ...
                    WorkSheet1.Cells[1, 5].Value = "Contractor Name";
                    WorkSheet1.Cells[1, 6].Value = "Department Name";
                    ...
                    foreach (var items in lstSizeObj)
                    {
                        ...
                        if (lstMUserObj.ToList().Count > 0)
                        {
                            foreach (var mitems in lstMUserObj)
                            {
                                ...
                                WorkSheet1.Cells[recordIndex, 5].Value = items.ContractorName;
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> ExportWorkerLiveReportData(WorkerSearch_Request parameters)
        {
            _response.IsSuccess = false;
            byte[] result;
            int recordIndex;
